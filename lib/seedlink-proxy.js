...
SeedlinkProxy.prototype.attachSocketEvents = function() {
  ...
  this.seedlinkSocket.on("connect", () => this._buffer = new Buffer(0));
  ...
  this.seedlinkSocket.on("data", function(data) {
    ...
    if(this._buffer.slice(0, SL_OK.length).equals(SL_OK)) {
    ...
    // Buffer may be larger than a single record when requesting past data
    while(this._buffer.length >= SL_RECORD_SIZE) {
      ...
      this.broadcast(this.unpackRecord(this._buffer.slice(SL_HEADER_SIZE, SL_RECORD_SIZE)));

      // Slice buffer beyond the record end
      this.sliceBuffer(SL_RECORD_SIZE);
  ...
  this.seedlinkSocket.on("close", () => this._connected = false);
...
SeedlinkProxy.prototype.broadcast = function(object) {
  ...
  */
  ...
  this._sockets.forEach(socket => socket.emit("write", object));
...
SeedlinkProxy.prototype.getStreamCommands = function() {
  ...
  function getPastTime() {
    ...
    // Extract padded parameters
    var year = now.getFullYear();

    // Month starts at 0 (Jan)
    var month = pad(1 + now.getMonth();
    var day = pad(now.getDate());
    var hour = pad(now.getHours());
    var minutes = pad(now.getMinutes());

    // Seedlink format YYYY,MM,DD,HH,MM
    return new Array(year, month, day, hour, minutes).join(",");

  }

  var commands = new Array("END");

  // Correct Seedlink handshake.. set sequence number to 000000
  this.selectors.forEach(function(stream) {
    commands.push("DATA 000000 " + getPastTime());
