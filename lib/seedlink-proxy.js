...
const SeedlinkProxy = function(options) {
  ...
  // Save a reference to the particular channel
...
SeedlinkProxy.prototype.removeSocket = function(socket) {
  ...
  socket.send(JSON.stringify({"success": "Unsubscribed from channel " + this.name + "."}));
...
SeedlinkProxy.prototype.addSocket = function(socket) {
  ...
  socket.send(JSON.stringify({"success": "Subscribed to channel " + this.name + "."}));
